...
<div class="content">
    ...
    <div class="main-content">
      <form id="attendanceForm" action="{{ route('admin.attendance.update', ['id' => $attendance->id]) }}" method="post">
        @csrf
        <div class="row">
          <p class="label">名前</p>
        ...
        <div class="row">
          ...
          <p class="value">
            <input class="input-value" id="start_time" name="start_time" value="{{ \Carbon\Carbon::parse($attendance->start_time)->format('H:i') }}" >~
            <input class="input-value" id="end_time" name="end_time" value="{{ \Carbon\Carbon::parse($attendance->end_time)->format('H:i') }}">
          </p>
        ...
        <div class="break-row" data-index="{{ $index }}">
          <p class="label">休憩 {{ $index + 1 }}</p>
          <p class="value">
            <input type="hidden" name="break_id[]" value="{{ $breakTime->id }}"> <!-- 休憩時間IDを隠しフィールドとして追加 -->
            <input class="input-value" id="break_start_time_{{ $index }}" name="break_start_time[]" value="{{ $breakTime->break_start_time ? \Carbon\Carbon::parse($breakTime->break_start_time)->format('H:i') : '' }}">~
            <input class="input-value" id="break_end_time_{{ $index }}" name="break_end_time[]" value="{{ $breakTime->break_end_time ?  \Carbon\Carbon::parse($breakTime->break_end_time)->format('H:i') : ''  }}">
          </p> 
        </div>
        @endforeach
        </div>
        ...
        <div class="button">
            ...
            <button class="btn" type="submit"  id="submitBtn" disabled onclick='return confirm("修正を確定しますか？")'>修正</button>
         </div>
